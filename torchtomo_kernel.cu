...
// from https://forums.developer.nvidia.com/t/atomicadd-kahan-summation/37155/3
// atomicAdd using Kahan/Babuska/Neumaier algorithm 
// this is necessary to avoid non-deterministic round-off errors arising from atomicAdd
__device__ void atomicAddKbn(float* __restrict__ address, float* __restrict__ address_c, float val)
{
	float oldacc = atomicAdd(address, val);	// Accumulate high-order bits
	float newacc = oldacc + val;		// Recover addition result
   float r = (fabsf(oldacc) >= fabsf(val)) ? ( (oldacc-newacc) + val ) : ((val - newacc) + oldacc ); //Fast2Sum
	atomicAdd(address_c, r);	// Accumulate low-order bits
}
...
// computes one backprojection view
__global__ void backprojection_view_kernel_kbn(
                    torch::PackedTensorAccessor32<float,3,torch::RestrictPtrTraits> image,
                    torch::PackedTensorAccessor32<float,3,torch::RestrictPtrTraits> imagec,
                    const torch::PackedTensorAccessor32<float,3,torch::RestrictPtrTraits> sinogram,
                    const float dx,
                    const float dy,
                    const float x0,
    ...
    if (xad>yad){   // loop through x-layers of image if xad>yad. This ensures ray hits only one or two pixels per layer
       ...
       for (int ix = 0; ix < nx; ix++){
          ...
          float pix_y_old = y0 + dy*(iyOld+0.5); // used to set mask
            if (iy == iyOld){ // if true, ray stays in the same pixel for this x-layer
             if ((iy >= 0) && (iy < ny) && (pix_x*pix_x + pix_y*pix_y <= fov_radius2)){
                atomicAddKbn(&image[ib][ix][iy],&imagec[ib][ix][iy],sinoval*travPixlen);
                // image[ib][ix][iy] += sinoval*travPixlen;
              }
          } else {    // else case is if ray hits two pixels for this x-layer
             float yMid = dy*max(iy,iyOld)+yl;
             ...
             float frac2 = 1.0-frac1;
             if ((iyOld >= 0) && (iyOld < ny) && (pix_x*pix_x + pix_y_old*pix_y_old <= fov_radius2)){
                 atomicAddKbn(&image[ib][ix][iyOld],&imagec[ib][ix][iyOld],frac1*sinoval*travPixlen);
                // image[ib][ix][iyOld] += frac1*sinoval*travPixlen;
              }
             if ((iy >= 0) && (iy < ny) && (pix_x*pix_x + pix_y*pix_y <= fov_radius2)) {
                atomicAddKbn(&image[ib][ix][iy],&imagec[ib][ix][iy],frac2*sinoval*travPixlen);
                // image[ib][ix][iy] += frac2*sinoval*travPixlen;
              }
          }
          iyOld=iy;
       ...
       for (int iy = 0; iy < ny; iy++){
          ...
          if (ix == ixOld){ // if true, ray stays in the same pixel for this y-layer
             ...
             float frac2=1.0-frac1;
             if ((ixOld >= 0) && (ixOld < nx) && (pix_x_old*pix_x_old + pix_y*pix_y <= fov_radius2)){
                atomicAddKbn(&image[ib][ixOld][iy],&imagec[ib][ixOld][iy],frac1*sinoval*travPixlen);
                // image[ib][ixOld][iy] += frac1*sinoval*travPixlen;
              }
             if ((ix >= 0) && (ix < nx) && (pix_x*pix_x + pix_y*pix_y <= fov_radius2)){
                atomicAddKbn(&image[ib][ix][iy],&imagec[ib][ix][iy],frac2*sinoval*travPixlen);
...
torch::Tensor circularFanbeamBackProjection_cuda_kbn(const torch::Tensor sinogram, const int nx, const int ny,
   ...
   const auto optionsc = torch::TensorOptions().dtype(sinogram.dtype()).device(sinogram.device());
   auto imagec = torch::zeros({batch_size, nx, ny}, optionsc);
   auto imagec_a = imagec.packed_accessor32<float,3,torch::RestrictPtrTraits>();
   ...
   backprojection_view_kernel_kbn<<<grid_dim, block_dim>>>(image_a,
                                               imagec_a,
                                               sinogram_a,
                                               dx,
                                               dy,
    ...
    return image+imagec; //Kahan correction
}
